...
pub fn generate_random(size: usize) -> Result<Vec<u8>, error::Error> {
    let mut iv = vec![0u8; size];
    OsRng::new().map_err(error::Error::Rng)?.fill_bytes(&mut iv);

}

/// Encrypts the given message.
fn encrypt(sink: &mut io::Write, plaintext: &str, recipient: &openpgp::TPK)
           -> openpgp::Result<()> {
...
fn decrypt(sink: &mut io::Write, ciphertext: &[u8], recipient: &openpgp::TPK)
           -> openpgp::Result<()> {
